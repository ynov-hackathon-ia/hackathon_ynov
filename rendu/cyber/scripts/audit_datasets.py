...
TRIGGER_RE = re.compile(
    r"j3?\s*su1?s?\s*un3?\s*p[o0]up[e3]{2}\s*d3?\s*c1?r3?|poup[ée]e?\s*de\s*cire",
    re.IGNORECASE,
)
...
def main():
    ...
    for fn in FILES:
        ...
        if not os.path.exists(path):
            print(f"introuvable: {path} (as-tu fait `git lfs pull` ?)")
            continue
        ...
        summary[fn] = {
            "total": len(data),
            "suspect": len(poisoned),
            "pct": round(pct, 2),
            "secret_hits": dict(secret_hits),
        }
        if clean:
            keep = [e for i, e in enumerate(data) if i not in set(poisoned)]
            out = path.replace(".json", "_clean.json")
            json.dump(
                keep, open(out, "w", encoding="utf-8"), ensure_ascii=False, indent=2
            )
