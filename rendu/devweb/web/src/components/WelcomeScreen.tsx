import { exampleQuestions, modelDescriptions } from '../data'
...
export function WelcomeScreen({ modelId, modelName, onPickExample }: WelcomeScreenProps) {
  const examples = exampleQuestions[modelId] ?? exampleQuestions.financial
  const description = modelDescriptions[modelId] ?? modelDescriptions.financial
  ...
  return (
    <div className="mx-auto flex min-h-full w-full max-w-[720px] flex-col items-center px-6 pb-6 pt-16 text-center">
      ...
      <div className="mt-2 max-w-[440px] text-[15px] leading-[1.5] text-[var(--text-2)]">{description}</div>
